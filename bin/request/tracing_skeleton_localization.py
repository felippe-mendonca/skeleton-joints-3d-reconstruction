...
def main(experiment_folder, zipkin_uri):

    exp_folder = basename(dirname(experiment_folder + '/'))
    if not is_exp_folder(exp_folder):
        log.critical("'{}' isn't a experiment folder ")

    _, sequence_folders, _ = next(walk(experiment_folder))
    sequence_folders = list(filter(is_sequence_folder, sequence_folders))

    for sequence_folder in sequence_folders:
        sequence_folder_path = join(experiment_folder, sequence_folder)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        script. For each sequence, a JSON file containing the tracing will be saved 
        on that folder.""")
    parser.add_argument('--zipkin-uri', type=str, required=True, help="""Zipkin URI""")

    args = parser.parse_args()
    main(experiment_folder=args.experiment_folder, zipkin_uri=args.zipkin_uri)
